...
def create_heatmap(frame_bgr, cavity_mask, metric='r'):
    """
...
def create_comparison_video(video_path, cavity_dir, output_path, start_frame=None, end_frame=None):
    """
    ...
    print(f"Creating comparison video for frames {start_frame}-{end_frame}")
    print(f"Found {len(frame_indices)} frames with masks")
    ...
    print(f"Output: {out_w}x{out_h} @ {fps:.1f} fps")
    ...
    for frame_idx in tqdm(frame_indices, desc="Creating video"):
        ...
        if not mask_path.exists():
            continue
        cavity_mask = cv2.imread(str(mask_path), cv2.IMREAD_GRAYSCALE) > 127
